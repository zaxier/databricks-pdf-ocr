...
class TestPDFSync:
    ...
    def test_determine_files_to_upload_all_new(self, temp_dir, sync_config, mock_workspace_client):
        ...
        files_to_upload = sync._determine_files_to_upload(
            local_files, remote_files, sync_config
        )
        
        # All local files should be uploaded
        assert len(files_to_upload) == len(local_files)
    
    def test_determine_files_to_upload_some_exist(self, temp_dir, sync_config, mock_workspace_client):
        ...
        remote_files = {
            f"/Volumes/test/test/test/test1_{test1_hash}.pdf": test1_hash
        }
    ...
    def test_sync_with_failures(self, mock_logger, temp_dir, sync_config, mock_workspace_client):
        ...
        mock_workspace_client.files.upload.side_effect = [
