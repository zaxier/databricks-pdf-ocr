...
def add_autoloader_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--backfill-interval", default="1 day", help="Interval for backfill operations"
    )

    parser.add_argument("--schema-location", help="Custom schema location for autoloader")
...
def create_configuration_dict(config_obj: Any) -> dict[str, Any]:
    ...
    if hasattr(config_obj, "__dataclass_fields__"):
        ...
        return {key: value for key, value in vars(config_obj).items() if not key.startswith("_")}
...
def ensure_spark_session_stopped(spark_session: Any) -> None:
    """
