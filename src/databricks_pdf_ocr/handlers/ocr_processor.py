...
from ..handlers.base import BatchResult, PDFHandler, PageResult, ProcessingResult
...
class OCRProcessor(PDFHandler):
    ...
    def process(self, **kwargs) -> ProcessingResult:
        ...
        try:
            ...
            for i in range(0, total_files, self.config.batch_size):
                ...
                # Log batch progress
                print(f"Batch {i//self.config.batch_size + 1} completed: "
    ...
    def _process_batch(self, files: List[Row], run_id: str) -> BatchResult:
        ...
        with ThreadPoolExecutor(max_workers=min(len(files), 5)) as executor:
            future_to_file = {
                executor.submit(self._process_single_pdf, file_row): file_row
                for file_row in files
            }
    ...
    def _resize_image(self, img: Image.Image, max_edge_pixels: int) -> Image.Image:
    ...
    def _store_page_results(self, page_results: List[PageResult], file_id: str, run_id: str) -> None:
        ...
        for page_result in page_results:
            results_data.append({
                ...
                "extraction_confidence": float(page_result.extraction_confidence) if page_result.extraction_confidence is not None else None,
                "processing_timestamp": datetime.now(),
                "processing_duration_ms": int(page_result.processing_duration_ms) if page_result.processing_duration_ms is not None else 0,
    ...
    def get_failed_files(self, limit: Optional[int] = None) -> DataFrame:
