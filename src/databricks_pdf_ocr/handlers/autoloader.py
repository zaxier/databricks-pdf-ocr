...
class AutoloaderHandler(PDFHandler):
    ...
    def start_stream(self) -> StreamingQuery:
        ...
        if self.config.file_format == "binaryFile":
            ...
            autoloader_options["cloudFiles.schemaHints"] = "path STRING, modificationTime TIMESTAMP, length LONG, content BINARY"
    ...
    def _process_batch(self, batch_df: DataFrame, batch_id: int) -> None:
        ...
        try:
            ...
            existing_files = (
                self.spark.table(self.config.source_table_name)
                .select("file_id")
                .distinct()
            )
            
            # Only insert new files
            new_files = batch_df.join(
                existing_files, 
                on="file_id", 
                how="left_anti"
            )
    ...
    def get_ingestion_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status_counts = (
                source_table.groupBy("processing_status")
                .count()
                .collect()
            )
            ...
            # Get total file size
            total_stats = source_table.agg(
                {"file_size": "sum", "file_id": "count"}
            ).collect()[0]
