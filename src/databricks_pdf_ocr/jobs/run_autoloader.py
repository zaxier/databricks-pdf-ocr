...
def run_autoloader(spark: SparkSession, config: AutoloaderConfig, mode: str = "stream") -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument(
        "--source-table-path",
        required=True,
        help="Path to source Delta table"
    )
    
    # Optional arguments
    parser.add_argument(
        "--mode",
        choices=["stream", "rescan", "stats"],
        default="stream",
        help="Execution mode"
    )
    ...
    parser.add_argument(
        "--file-format",
        default="binaryFile",
        help="File format for autoloader"
    )
    
    parser.add_argument(
        "--cleanup-checkpoint",
        action="store_true",
        help="Clean up checkpoint before starting"
    )
