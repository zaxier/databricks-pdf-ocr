...
def run_ocr_processing(
    spark: SparkSession, config: OCRConfig, command: str = "process"
) -> dict[str, Any]:
    ...
    try:
        if command == "process":
            ...
            return {"success": True, "command": command, "result": result.get_summary()}

        elif command == "stats":
            # Get processing statistics
            stats = processor.get_processing_stats()
            return {"success": True, "command": command, "stats": stats}
            ...
            return {"success": True, "command": command, "reset_count": count}

        elif command == "test-claude":
            # Test Claude API connection
            test_result = processor.test_claude_connection()
            return {"success": True, "command": command, "test_result": test_result}
...
def main() -> None:
    ...
    parser.add_argument(
        "--target-table-path", required=True, help="Path to target Delta table for OCR results"
    )

    parser.add_argument(
        "--state-table-path", required=True, help="Path to state Delta table for run tracking"
    )
    ...
    try:
        ...
        # Show what would be processed for dry run
        if args.dry_run:
            ...
            if file_count > 0:
                ...
                for row in pending_files.select(
                    "file_name", "processing_status", "processing_attempts"
                ).collect():
                    print(
                        f"  - {row.file_name} (status: {row.processing_status}, attempts: {row.processing_attempts})"
                    )
        ...
        if result["success"]:
            ...
            if "failed_files" in result:
                ...
                for failed_file in result["failed_files"][:10]:  # Show first 10
                    print(
                        f"  - {failed_file['file_name']} ({failed_file['attempts']} attempts): {failed_file['error']}"
                    )
            ...
            if "test_result" in result:
                ...
                print(
                    f"Claude API test: {test['status']} (response time: {test['response_time_ms']}ms)"
                )
                if test.get("error"):
